...
class ShipmentService
{
    ...
    public function createShipment(ShipmentRequest $request): ServiceResult
    {
        try {
            ...
            if (! $trackingNumber) {
                return ServiceResult::fail('aras_no_tracking', __('cargo.no_tracking'));
            }

            return ServiceResult::ok(['tracking_number' => $trackingNumber, 'label_url' => null]);
        } catch (SoapFault $e) {
            return ServiceResult::fail('aras_soap_error', $e->getMessage());
        }
    }
    ...
    public function cancelShipment(string $trackingNumber): ServiceResult
    {
        try {
            ...
            return ServiceResult::fail('aras_cancel_error', $e->getMessage());
        }
    }
     ...
     */
    public function getLabel(string $trackingNumber, string $format = 'pdf'): ServiceResult
    {
        try {
            ...
            if (! $labelData) {
                return ServiceResult::fail('aras_label_empty', __('cargo.label_empty'));
            }

            return ServiceResult::ok($labelData);
        } catch (SoapFault $e) {
            return ServiceResult::fail('aras_label_error', $e->getMessage());
        }
    }
}
