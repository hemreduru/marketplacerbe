...
class SyncUserMarketplaceDataCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $credentials = \App\Models\UserMarketplaceCredential::where('user_id', $user->id)->get();
        ...
        foreach ($credentials as $credential) {
            ...
            try {
                $orderService = new \App\Services\Trendyol\TrendyolOrderService(
                    $credential->api_key,
            ...
            try {
                $productService = new \App\Services\Trendyol\TrendyolProductService(
                    $credential->api_key,
                    $credential->api_secret,
                    $credential->additional_credentials['seller_id'] ?? '',
                    false
            ...
            try {
                $financeService = new \App\Services\Trendyol\TrendyolFinanceService(
                    $credential->api_key,
                    $credential->api_secret,
                    $credential->additional_credentials['seller_id'] ?? '',
                    false
